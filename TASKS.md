...
## ▶️ Doing now
- [ ] **New Blood Discovery form (OHDBalt edition)** — ✅ BUILT & VERIFIED (all 9 tasks, 16 commits `0d2e65f3..aa200b41`). Live in dev at /discovery/overhead-door: focused-canvas page, progressive dual-axis gap-map sliders (handling reveals at importance ≥7), custom `wp_nb_discovery_responses` table, gap-map-first summary email to joms@newblood.com, config-driven for reuse. Per-task + final whole-feature review passed; hardened (nocache, honeypot + per-IP throttle, mail-failure logging); verified e2e incl. real-browser Playwright. Draft email: `docs/clients/ohdbalt-discovery-email-DRAFT.md`. **▶️ Next: Jeremy review → SFTP deploy to prod → confirm prod SMTP delivers (no SMTP locally) → `wp rewrite flush` → send Chase the link.** _(feeds the OHDBalt comprehensive proposal)_
