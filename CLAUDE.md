...
The `/pricing` page is two doors. Door one: Starter $3,500 / Business $5,000 plus the "Already have a site?" Tune / Tune Plus row. Door two (added 2026-09-02): Four Gaps Audit $1,500-$2,500 / Territory Platform $35,000 + $3,200/mo (Flagship) / Operations Platform "from $90,000" build plus a monthly scoped with it (shape doc: `docs/products/operations-platform-shape-2026-09.md`, 2026-09-03; one record for the whole business, connected or owned mode, four agents, portal, phased on top of a live Territory). The `/four-gaps-audit` page (pattern `newblood/four-gaps-audit`, added 2026-09-03) is the paid-discovery page; its button and the pricing/Territory audit buttons go to `/contact/?subject=four-gaps-audit`, a theme-owned preselect handled in `functions.php` (`newblood_contact_subjects()`; `operations-platform` is the other key). The `/territory` page (pattern `newblood/territory`) is the flagship offer page; the homepage has a `newblood/two-doors` section directly below the hero (the `statement` pattern is no longer in `front-page.html` but stays on disk). Territory is Build + Manage + the content program packaged for established operators, not a fifth service. Plan: `docs/newblood-territory-repositioning-2026-09.md`. The header nav is inline in `parts/header.html` (the wp_navigation posts 6737/6738/6739 are not referenced by the header); add nav items there.

The Signal AI Visibility Audit is a separate by-appointment service line that lives quietly on the About page only — see `docs/superpowers/specs/2026-04-26-signal-sales-surface-design.md`. Its product (the audit tool itself) is in a separate repo at `/Users/jeremyoms/newblood-signal/`. Its intro pattern (`signal-intro`) now points at `/four-gaps-audit/`, but the pattern is not placed on any page (About holds `about-story` + `why-new-blood`); placing it is Jeremy's call.
