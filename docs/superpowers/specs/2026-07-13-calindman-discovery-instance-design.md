...
- Verification, precise bar (amended 2026-07-13 during planning — the module's three consumers render the seven questions in three different historical orders, so literal byte-identity across all of them is impossible with a single config order; config adopts the *form's* order):
  ...
  - **Stored payloads:** sanitize output for a fixed raw OHDBalt payload is value-identical pre/post (locked by a fixture test; compared strictly after canonical key sort — associative key *order* inside the stored JSON is not API since every consumer accesses by key name, and the old code's payload order differed from the form's display order, so both couldn't be preserved by one config. Decided with Jeremy 2026-07-13: form order governs).
