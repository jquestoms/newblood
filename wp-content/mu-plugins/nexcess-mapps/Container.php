...
class Container {
		...
		return array_merge( [
			...
			Commands\Cache::class                       => function ( $app ) {
				return new Commands\Cache(
					...
					$app->get( Integrations\Cache::class )
				);
			},
			...
			Commands\Robots::class                   => function ( $app ) {
				return new Commands\Robots(
					$app->get( ContainerPF::class )->get( Modules\Robots::class )
				);
			},
	 ...
	 * @param string $class_name The integration class name.
	 *
	 * @return object The integration instance.
	 *
	 * @codeCoverageIgnore
	 */
	private function buildIntegration( $app, $class_name ) {
		return new $class_name( $app->get( Settings::class ) );
	}
}
