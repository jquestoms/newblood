...
class Support extends Command {
	...
	public function featureFlags( $args, $assoc_args ) {
		...
		// Refresh the remote flag data if needed.
		if ( ! empty( $assoc_args['refresh'] ) ) {
			$this->flags->getCurrentFeatureFlags( true );
		}

		$action        = ! empty( $args[0] ) ? $args[0] : '';
		$flag          = ! empty( $args[1] ) ? $args[1] : '';
		$current_flags = $this->flags->getCurrentFeatureFlags();
		...
		switch ( $action ) {
			case 'enable':
				if ( empty( $flag ) ) {
					WP_CLI::warning( 'A flag must be specified.' );
					return;
				}
			...
			default:
				if ( ! empty( $assoc_args['all'] ) ) {
					if ( empty( $cohorts ) && empty( $current_flags ) ) {
						WP_CLI::line( 'Feature flag data not yet available.' );
						return;
					}
					self::section( 'Flags (remote)', $current_flags, 'feature_flags' );
					self::section( 'Cohorts', $cohorts, 'feature_flags' );
					WP_CLI::line();
					return;
				}
				if ( empty( $flag ) ) {
					WP_CLI::warning( 'A flag must be specified. Alternatively, you can use the --all flag to view all feature flags.' );
					return;
				}
				$enabled = $this->flags->enabled( $flag );
