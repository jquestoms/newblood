...
abstract class Command {
	 ...
	 * @see WP_CLI::colorize()
	 *
	 * @param string $value A string to colorizing, using one or more color tokens.
	 *
	 * @return string The colorized string.
	 */
	protected function colorize( $value ) {
		return WP_CLI::colorize( $value );
	}
	 ...
	 * @param bool|int        $should_exit Optional. Exit the script automatically, optionally specifying
	 *                                     the exit code. Default is true (exit code 1).
	 *
	 * @return self
	 */
	protected function error( $message, $should_exit = true ) {
		if ( is_array( $message ) ) {
			WP_CLI::error_multi_line( $message );

			if ( $should_exit ) {
				$this->halt( (int) $should_exit );
			}
		} else {
			WP_CLI::error( $message, $should_exit );
		}
	...
	protected function step( $message ) {
		$this->log( $this->colorize( sprintf( '%%c%1$s%%n', $message ) ) );
		return $this;
	}
