 ...
 * @property-read bool   $is_lite                         TRUE if this is a "lite" plan.
 ...
 * @property-read bool   $plugin_updates_enabled          Opposite of $mapps_plugin_updates_enabled
 * 														  FALSE if MAPPS is responsible for automatic plugin updates,
 *                                                        TRUE if the responsibility falls to WordPress core.
...
class SettingsPF extends BaseSettings {
	...
	protected function loadSettings( array $settings ) {
		...
		$plan_type              = ! empty( $this->getSiteWorxSetting( 'app_type_actual', false ) ) ? $this->getSiteWorxSetting( 'app_type_actual' ) : $this->getSiteWorxSetting( 'app_type', 'unknown' );
		$plugin_updates_enabled = isset( $this->loadSiteWorx()['is_visual_compare_disabled'] )
			? $this->getSiteWorxSetting( 'is_visual_compare_disabled' )
			: ! $this->getSiteWorxSetting( 'app_updates_plugin', false );
		...
		$defaults = [
			...
			'plugin_updates_enabled'         => (bool) $plugin_updates_enabled,
		...
		$settings = array_merge( $defaults, [
			...
			'is_lite'              => in_array( $defaults['package_label'], self::LITE_PLANS, true ),
