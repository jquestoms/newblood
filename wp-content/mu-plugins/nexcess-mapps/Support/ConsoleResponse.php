...
class ConsoleResponse {
	 ...
	 *
	 * @param bool $should_throw Optional. If true, a ConsoleException will be thrown if the command was
	 *                           unsuccessful. Default is false.
	 *
	 * @throws \StellarWP\PluginFramework\Exceptions\ConsoleException If $should_throw is true and the command exited
	 *                                                                with a non-zero exit code.
	 *
	 * @return bool True if the command was successful (a zero exit code) or false if something
	 *              went wrong.
	 */
	public function wasSuccessful( $should_throw = false ) {
		$successful = 0 === $this->exit_code;

		if ( $should_throw && ! $successful ) {
