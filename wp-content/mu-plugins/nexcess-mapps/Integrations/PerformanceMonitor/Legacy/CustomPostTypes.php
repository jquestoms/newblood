...
namespace Nexcess\MAPPS\Integrations\PerformanceMonitor\Legacy;
...
class CustomPostTypes {
	...
	public static function getMetaFieldsDefinitions() {
		...
		$required_object_string_property  = [
			'type'     => 'string',
			'required' => true,
		];
		$optional_object_integer_property = [
			'type' => 'integer',
		];
		$required_object_integer_property = [
			'type'     => 'integer',
			'required' => true,
		];
		...
		return [
			self::POST_TYPES['report']      => [
				...
				'global_performance' => self::getMetaTypeDefinition( 'object', [
					'type'              => 'array',
					'sanitize_callback' => null,
					'show_in_rest'      => [
						'schema' => [
							...
							'items' => [
								'type'       => 'object',
								'properties' => [
									'region'               => $required_object_string_property,
									'url'                  => $required_object_string_property,
									'score'                => $required_object_integer_property,
									'load_time'            => $optional_object_integer_property,
									'lcp_time'             => $optional_object_integer_property,
									'max_fid'              => $optional_object_integer_property,
									'render_blocking_time' => $optional_object_integer_property,
									'total_blocking_time'  => $optional_object_integer_property,
									'bootup_time'          => $optional_object_integer_property,
									'weight'               => $optional_object_integer_property,
								], // Ends `properties`.
							], // Ends `items`.
						], // Ends `schema`.
					], // Ends `show_in_rest`.
