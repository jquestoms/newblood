...
class ExtrapolationRevenue extends BaseInsightTypeGenerator {
	...
	public function generate() {
		$current_load_times = array_map(
			function ( PageDTO $page_dto ) {
				return $this->getPageMetric( $page_dto, 'load_time', 0 );
			},
			$this->pageDTOs
		);
		...
		$variable_dto     = new VariableDTO( 'revenue_lost', $revenue_lost_w_currency );
		$insight_data_dto = new InsightDataDTO( [ $variable_dto ] );
