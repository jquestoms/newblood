...
class WooCommerceAutomatedTesting extends Integration {
	...
	/**
	 * WooCommerce test product name.
	 */
	const WC_TEST_PRODUCT_NAME = 'WooCommerce Automated Testing Product';
	 ...
	 */
	public function __construct( Settings $settings, RouteManager $route_manager, Options $options, Logger $logger ) {
	...
	protected function getActions() {
		return [
			...
			[ Maintenance::WEEKLY_MAINTENANCE_CRON_ACTION, [ $this, 'cleanupWooCommerceTestProducts' ] ],
	...
	/**
	 * Accesses database directly for all the WC Test Products and returns an array of its ids.
	 *
	 * @return array<int>
	 */
	protected function getMultipleTestProductIds() {
		global $wpdb;

		// Execute the query and get the product IDs.
	 ...
	 * @param mixed        $prev_value Previous value, most likely true or false.
	 */
	public function enableOrDisable( $key, $new_value, $prev_value ) {
		...
		$this->forceEnableOrDisable( (bool) $new_value );
	}
	...
	/**
	 * Removes all the testing products except one.
	 *
	 * @param int $exclude The id of the product to exclude from the deletion.
	 */
	public function cleanupWooCommerceTestProducts( $exclude = 0 ) {
		global $wpdb;

		$product_ids = $this->getMultipleTestProductIds();

		if ( count( $product_ids ) <= 1 ) {
			return;
		}

		// Exclude the  product from deletion to keep it.
		if ( $exclude ) {
			$key = array_search( $exclude, $product_ids, true );
			if ( false !== $key ) {
	...
	/**
	 * Public method for retrieving recent results from the SaaS.
	 *
		...
		return CacheRemember::remember_transient( self::RESULTS_CACHE_KEY, function () {
			...
			try {
				...
				$this->logger->error( sprintf(
					...
					__( 'Bad request to WCAT service: %1$s', 'nexcess-mapps' ),
					$e->getMessage()
				) );
				return null;
			}
	...
	protected function createTestProduct() {
		...
		if ( ! $product ) {
			...
			$product->set_name( self::WC_TEST_PRODUCT_NAME );
