...
class PageCache extends Integration {
	...
	public function setup() {
		if (
			...
			|| is_multisite()
			|| $this->isPluginActive( 'w3-total-cache/w3-total-cache.php' )
			|| $this->isPluginBeingActivated( 'w3-total-cache/w3-total-cache.php' )
		) {
	...
	protected function getActions() {
		...
		if ( $this->isPluginActive( 'woocommerce/woocommerce.php' ) ) {
			$actions[] = [ 'plugins_loaded', [ $this, 'moveWooPackagesInitHook' ], 1 ];
		}
	...
	public function cleanCacheEnablerWpConfig() {
		...
		$fs = Filesystem::init();
		if ( ! $fs->exists( $file ) || ! $fs->is_writable( $file ) ) {
			return;
		}
		...
		if ( $contents !== $updated ) {
			$fs->put_contents( $file, (string) $updated, LOCK_EX );
		}
	}

	/**
	 * Move the Automattic\WooCommerce\Packages::on_init() callback to a later hook.
	 * WooCommerce runs package migrations too early (plugins_loaded) for access to
	 * the global $wp_rewrites object. This fixes that problem.
	 */
	public function moveWooPackagesInitHook() {
		remove_action( 'plugins_loaded', [ 'Automattic\WooCommerce\Blocks\Package', 'init' ] );
		remove_action( 'init', [ 'Automattic\WooCommerce\Blocks\Package', 'init' ] );
	}
	 ...
	 *
	 * @param mixed[] $old_value The previous configuration.
	 * @param mixed[] $new_value The new configuration.
	 */
	public function settingsUpdated( $old_value, $new_value ) {
		Cache_Enabler_Disk::create_settings_file( $new_value );
		 ...
		 */
		if ( isset( $old_value['enabled'] ) && $old_value['enabled'] !== $new_value['enabled'] ) {
			try {
				if ( 1 === $new_value['enabled'] ) {
		 ...
		 */
		if (
			isset( $new_value['enabled'] ) && 1 === $new_value['enabled']
