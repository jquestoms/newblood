...
class Feature_Modal extends Component {
	...
	public function get_dashboard_modals( $force_hide = false ): array {
		...
		$title             = esc_html__( 'New! AntiBot Firewall Protection', 'wpdef' );
		...
		$desc .= $is_site_connected ?
			esc_html__( 'Activate now for automatic, 24/7 protection and stronger, hassle-free security.', 'wpdef' ) :
			esc_html__( 'Sign up for a free WPMU DEV account to activate advanced firewall protection today.', 'wpdef' );
		$desc .= '</p>';

		$desc .= '<ul class="feature-highlights">
			<li><span class="sui-icon-check-tick" aria-hidden="true"></span>' . esc_html__( 'Dynamic IP Blocklist', 'wpdef' ) . '</li>
			<li><span class="sui-icon-check-tick" aria-hidden="true"></span>' . esc_html__( 'Automated Firewall Protection', 'wpdef' ) . '</li>
			<li><span class="sui-icon-check-tick" aria-hidden="true"></span>' . esc_html__( 'Millions of IPs Checked', 'wpdef' ) . '</li>
			<li><span class="sui-icon-check-tick" aria-hidden="true"></span>' . esc_html__( 'Regularly Reviewed IP Database', 'wpdef' ) . '</li>
		</ul>';

		$button_title      = $is_site_connected ?
			esc_html__( 'Activate antibot firewall', 'wpdef' ) :
			esc_html__( 'Connect site to Activate for free', 'wpdef' );
		...
		return array(
			...
			'welcome_modal'      => array(
				...
				'read_more_title'     => esc_html__( 'LEARN MORE', 'wpdef' ),
