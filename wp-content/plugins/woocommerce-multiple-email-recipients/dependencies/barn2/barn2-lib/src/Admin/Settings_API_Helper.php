...
namespace Barn2\Plugin\WC_Multiple_Email_Recipients\Dependencies\Lib\Admin;
...
class Settings_API_Helper implements Registerable, Conditional
{
    ...
    private static function field_description($args)
    {
        if (!empty($args['desc'])) {
            ...
            if ($is_page_select) {
                $args['desc'] .= ' <a href="' . \esc_url(\get_permalink($args['show_page'])) . '" target="_blank">' . esc_html__('[View page]', 'woocommerce-multiple-email-recipients') . '</a>';
            }
    ...
    public static function settings_field_color($args)
    {
        ...
        esc_attr_e($args['input_class'],'woocommerce-multiple-email-recipients' );
        ?>">
			<input
				type="text"
				name="<?php 
    ...
    public static function settings_field_color_size($args)
    {
        ...
        $size_placeholder = !empty($args['placeholder']) ? $args['placeholder'] : __('Size', 'woocommerce-multiple-email-recipients');
        if (empty($args['custom_attributes'])) {
