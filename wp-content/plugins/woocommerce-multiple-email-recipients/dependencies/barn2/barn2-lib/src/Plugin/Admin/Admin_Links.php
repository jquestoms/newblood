...
class Admin_Links implements Registerable, Standard_Service
{
    ...
    public function add_settings_link($links)
    {
        ...
        \array_unshift($links, \sprintf('<a href="%1$s">%2$s</a>', \esc_url($settings_url), __('Settings', 'woocommerce-multiple-email-recipients')));
        return $links;
    }
    public function add_documentation_link($links, $file)
    {
        ...
        $row_meta = ['docs' => \sprintf(
            ...
            \esc_attr(\sprintf(__('View %s documentation', 'woocommerce-multiple-email-recipients'), $this->plugin->get_name())),
            esc_html__('Docs', 'woocommerce-multiple-email-recipients')
        )];
