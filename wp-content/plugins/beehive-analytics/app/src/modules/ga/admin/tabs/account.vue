...
export default {
	...
	methods: {
		...
		formSubmit() {
			this.handleErrors()
			if (!this.error.measurement && !this.error.stream) {
				// Save settings.
				this.$emit('submit')
		...
		/**
		 * Handle and set error states for stream and measurement.
		 *
		 * Sets the stream error to true if no stream is selected or if the selection is invalid.
		 * Sets the measurement error based on the validity of the measurement.
		 *
		 * @since 3.4.14
		 */
		handleErrors() {
			const selectedStream = this.getOption('stream', 'google')
			this.error.stream =
				selectedStream === '' || selectedStream === false
