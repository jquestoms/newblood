...
class GW_GoPricing_Helper {
    ...
    public static function clean_input( $data = [], $format = 'filtered', $exclude_html_keys = [], $exclude_all_keys = [] ) {
    ...
    public static function remove_input( $data = [], $keys = [], $clean_private = true ) {
        foreach ( (array) $data as $key => $value ) {
            if ( is_array( $value ) ) {
    ...
    public static function fix_unesc_atts( $content ) {
        if ( strstr( '#go-pricing-table', $content ) ) {
            return $content;
        }
    ...
    public static function has_shortcode( $tag, $text = '' ) {
        ...
        // Search in content
        if ( $post instanceof WP_Post && GW_GoPricing_Helper::find_shortcode( $post->post_content, $tag, $text ) ) {
            return true;
        }
        ...
        if ( defined( 'MFN_OPTIONS_DIR' ) ) {
            ...
            if ( !empty( $meta ) && GW_GoPricing_Helper::find_shortcode( $meta, $tag, $text ) ) {
                return true;
            }
    ...
    public static function find_shortcode( $content, $tag, $text = '' ) {
        ...
        if ( empty( $text ) || !is_string( $text ) && !is_array( $text ) ) {
            return false;
        }
