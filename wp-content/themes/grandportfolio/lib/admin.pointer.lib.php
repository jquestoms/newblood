...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'grandportfolio-pointer-registration-dismissed',
				true
			)
		):
					...
					jQuery('#pp_panel_registration_a').first().pointer( 
						{
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'grandportfolio-pointer-registration',
...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'grandportfolio-pointer-required-plugins-dismissed',
				true
			)
		):
					...
					jQuery('#menu-appearance').first().pointer( 
						{
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'grandportfolio-pointer-required-plugins',
	...
	add_action(
		...
		function() {
			...
			if (
				! get_user_meta(
					get_current_user_id(),
					'grandportfolio-pointer-loftloader-dismissed',
					true
				)
			):
						...
						jQuery('#menu-settings').first().pointer( 
							{
								...
								close: function() {
									jQuery.post(
										ajaxurl,
										{
											pointer: 'grandportfolio-pointer-loftloader',
