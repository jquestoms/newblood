...
function grandportfolio_pagination($pages = '', $range = 4)
{
	 ...
	 if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."'>&laquo; ".esc_html__('First', 'grandportfolio')."</a>";
	 if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."'>&lsaquo; ".esc_html__('Previous', 'grandportfolio')."</a>";
	 ...
	 if ($paged < $pages && $showitems < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\">".esc_html__('Next', 'grandportfolio')." &rsaquo;</a>";
	 if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."'>".esc_html__('Last', 'grandportfolio')." &raquo;</a>";
	 echo "</div>";
	 }
}
		...
		<div class="gravatar">
      	...
      	<div class="right">
			<?php if ($comment->comment_approved == '0') : ?>
         		<em><?php echo esc_html_e('(Your comment is awaiting moderation.)', 'grandportfolio') ?></em>
         		<br />
      		<?php endif; ?>
			...
			<div class="comment_date"><?php echo date(THEMEDATEFORMAT, strtotime($comment->comment_date)); ?> <?php echo esc_html_e('at', 'grandportfolio') ?> <?php echo date(THEMETIMEFORMAT, strtotime($comment->comment_date)); ?></div>
			<?php 
      			if($depth < 3)
      			{
      		?>
      			<?php comment_reply_link(array_merge( $args, array('depth' => $depth,
'reply_text' =>  esc_html__('Reply', 'grandportfolio'), 'login_text' => esc_html__('Login to Reply', 'grandportfolio'), 'max_depth' => $args['max_depth']))) ?>
			<?php
				}
			?>
			<br class="clear"/>
...
function grandportfolio_posts($sort = 'recent', $items = 3, $echo = TRUE, $show_thumb = TRUE) 
{
	...
	if($sort == 'recent')
	{
		...
		$title = esc_html__('Recent Posts', 'grandportfolio');
	}
	else
	{
		...
		$title = esc_html__('Popular Posts', 'grandportfolio'); 
	}
...
function grandportfolio_get_image_id($url) 
{
...
{	
	...
	$results = array();
	$queries = array(
	'content' =>		array("UPDATE $wpdb->posts SET post_content = replace(post_content, %s, %s)",  esc_html__('Content Items (Posts, Pages, Custom Post Types, Revisions)','grandportfolio') ),
	'excerpts' =>		array("UPDATE $wpdb->posts SET post_excerpt = replace(post_excerpt, %s, %s)", esc_html__('Excerpts','grandportfolio') ),
	'attachments' =>	array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s) WHERE post_type = 'attachment'",  esc_html__('Attachments','grandportfolio') ),
	'links' =>			array("UPDATE $wpdb->links SET link_url = replace(link_url, %s, %s)", esc_html__('Links','grandportfolio') ),
	'custom' =>			array("UPDATE $wpdb->postmeta SET meta_value = replace(meta_value, %s, %s)",  esc_html__('Custom Fields','grandportfolio') ),
	'guids' =>			array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s)",  esc_html__('GUIDs','grandportfolio') )
	);
...
function grandportfolio_hex_darker($rgb, $darker=2)
{
...
function grandportfolio_themegoods_action() 
{
	$pp_verified_envato_grandportfolio = get_option('pp_verified_envato_grandportfolio');
	
	if(!$pp_verified_envato_grandportfolio OR empty($pp_verified_envato_grandportfolio))
	{
		if(defined('THEMEGOODS') && THEMEGOODS)
		{
			update_option("pp_verified_envato_grandportfolio", true);
...
function grandportfolio_get_default_recent_posts($items = -1) 
{
	//Get blog posts
	$args = array(
	    'numberposts' => $items,
...
function grandportfolio_check_instagram_authorization() {
...
function grandportfolio_get_instagram_using_plugin($cache_prefix = 'photostream', $limit = 30) {
	...
	//delete_option('grandportfolio_'.$cache_prefix.$limit);
	
	//Get Instagram cached data
	$instagram_cache_data = get_option('grandportfolio_'.$meks_instagram_settings['user_id'].$limit);
	...
	if(!isset($instagram_cache_data['data']) OR empty($instagram_cache_data['data']) OR $instagram_cache_timer > 30000)
	{
		...
		if(!empty($photos_arr))
		{
			...
			update_option('grandportfolio_'.$meks_instagram_settings['user_id'].$limit, $instagram_cache_data);
...
function grandportfolio_register_theme($purchase_code = '') {
...
function grandportfolio_unregister_theme() {
	$result = delete_option("envato_purchase_code_".ENVATOITEMID);
	return $result;
}

function grandportfolio_get_site_domain()
{
	$site_url = site_url();
