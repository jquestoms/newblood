...
function grandportfolio_add_admin() 
{
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'functions.php') {
		...
		if ( isset($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
				...
				if(!empty($_REQUEST['pp_envato_personal_token']) && strlen($_REQUEST['pp_envato_personal_token']) == 36) {
					...
					if(is_bool($response_obj->response_code)) {
						if($response_obj->response_code) {
							...
							grandportfolio_register_theme($_REQUEST['pp_envato_personal_token']);
			...
			$advance_cache_custom_css = get_option(SHORTNAME.'_advance_cache_custom_css');
			...
			if($advance_cache_custom_css)
			{
				...
				if(!file_exists($css_cache_path))
				{
					...
					$grandportfolio_custom_css = ob_get_contents(); 
					ob_end_clean();
					
					//Writing cache file
					file_put_contents($css_cache_path, $grandportfolio_custom_css);
				}
			}
	...
	add_menu_page('Theme Setting', 'Theme Setting', 'administrator', 'functions.php', 'grandportfolio_admin', '', 3);
}
...
function grandportfolio_enqueue_front_page_scripts() 
{
	...
	else
	{
		...
		if($advance_cache_custom_css)
		{
			...
			wp_enqueue_style("grandportfolio-custom-css", $cache_url."custom-css.css", false, "", "all");
		}
		else
		{
...
{ 
	...
	?>
		...
		<div id="pp_loading"><span><?php esc_html_e('Updating...', 'grandportfolio' ); ?></span></div>
		<div id="pp_success"><span><?php esc_html_e('Successfully Update', 'grandportfolio' ); ?></span></div>
		...
		<div class="header_wrap">
			...
			<h2><?php esc_html_e('Theme Setting', 'grandportfolio' ); ?><span class="pp_version">v<?php echo THEMEVERSION; ?></span></h2>
			</div>
			<div style="float:right;margin:20px 0 0 0">
				<input id="save_ppsettings" name="save_ppsettings" class="button button-primary button-large" type="submit" value="<?php esc_html_e('Save', 'grandportfolio' ); ?>" />
				<br/><br/>
	...
	?>
		...
		{
		?>
			<?php esc_html_e('Below are fonts that already installed.', 'grandportfolio' ); ?><br/>
	...
	?>
		...
		?>
			...
			<div id="<?php echo esc_attr($value['id']); ?>_wrapper" style="width:380px;font-size:11px;"><br/>
				...
				<a href="<?php echo esc_url($url); ?>" class="image_del button" rel="<?php echo esc_attr($value['id']); ?>"><?php esc_html_e('Delete', 'grandportfolio' ); ?></a>
			</div>
			<?php
				}
				else
			...
			<div id="<?php echo esc_attr($value['id']); ?>_wrapper" style="width:380px;font-size:11px;">
				...
				<?php esc_html_e('Listen current music', 'grandportfolio' ); ?></a>&nbsp;<a href="<?php echo esc_url($url); ?>" class="image_del button" rel="<?php echo esc_attr($value['id']); ?>"><?php esc_html_e('Delete', 'grandportfolio' ); ?></a>
			</div>
		<?php
				}
			}
	...
	?>
		...
		<select class="pp_font" data-sample="<?php echo esc_attr($value['id']); ?>_sample" data-value="<?php echo esc_attr($value['id']); ?>_value" name="<?php echo esc_attr($value['id']); ?>"
			id="<?php echo esc_attr($value['id']); ?>">
			<option value="" data-family="">---- <?php esc_html_e('Theme Default Font', 'grandportfolio' ); ?> ----</option>
			<?php 
				foreach ($pp_font_arr as $key => $option) { ?>
			<option
			<?php if (get_option( $value['id'] ) == $option['css-name']) { echo 'selected="selected"'; } ?>
		...
		<br/><br/><div id="<?php echo esc_attr($value['id']); ?>_sample" class="pp_sample_text"><?php esc_html_e('Sample Text', 'grandportfolio' ); ?></div>
		</div>
		<small><?php echo stripslashes($value['desc']); ?></small>
		<div class="clearfix"></div>
		</div>
...
?>
			...
			<div class="ppb_inline_wrap">
				...
				<a id="save_<?php echo esc_attr($_GET['rel']); ?>" data-parent="ppb_inline_<?php echo esc_attr($selected_shortcode); ?>" class="button-primary ppb_inline_save" href="javascript:;"><?php esc_html_e('Update', 'grandportfolio' ); ?></a>
			...
			<div class="ppb_inline_option_wrap">
				...
				<div class="ppb_inline_option">
					<label for="<?php echo esc_attr($selected_shortcode); ?>_title"><?php esc_html_e('Title', 'grandportfolio' ); ?></label><br/>
					...
					<span class="label_desc"><?php esc_html_e('Enter Title for this content', 'grandportfolio' ); ?></span>
				</div>
				<br/>
				<?php
					}
				...
				<div class="ppb_inline_option">
					...
					<br/><a id="video_view_<?php echo esc_attr($selected_shortcode); ?>_<?php echo esc_attr($attr_name); ?>" class="button ppb_video_url" target="_blank"><?php echo esc_html__('View Video', 'grandportfolio' ); ?></a>
				...
				?>
					<div class="ppb_inline_option">
						<label for="<?php echo esc_attr($selected_shortcode); ?>_content"><?php esc_html_e('Content', 'grandportfolio' ); ?></label><br/>
						...
						<span class="label_desc"><?php esc_html_e('You can enter text, HTML for its content', 'grandportfolio' ); ?></span>
		...
		jQuery(document).ready(function(){
			...
			jQuery("#save_<?php echo esc_js($_GET['rel']); ?>").click(function(){
			    ...
			    var targetItem = jQuery('#ppb_inline_current').val();
			    ...
			    jQuery("#"+parentInline+" :input.ppb_input").each(function(){
			     	...
			     	{	
				    	 ...
				    	 {
				    	  	jQuery('#'+targetItem).find('.title').html(decodeURI(jQuery(this).val()));
...
?>
	<div class="ppb_inline_wrap preview">
	    <h2><?php esc_html_e('Preview', 'grandportfolio' ); ?> <?php echo urldecode($page_title); ?></h2>
	    <a class="button button-primary" href="javascript:;" onClick="jQuery.fancybox.close();"><?php esc_html_e('Close', 'grandportfolio' ); ?></a>
	</div>	
...
function grandportfolio_import_demo_content() {
	if(is_admin() && isset($_POST['demo']) && !empty($_POST['demo']))
	{
	    ...
	    switch($_POST['demo'])
	    {
			...
			case 6:
			    //Create empty menu first before importing
			    $main_menu_exists = wp_get_nav_menu_object('Main Menu2');
			    if(!$main_menu_exists)
			    {
				    $main_menu_id = wp_create_nav_menu('Main Menu2');
			    }
			break;
	    }
	    ...
	    { 
	    	switch($_POST['demo'])
	    	{
		    	...
		    	default:
		    		...
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/landing';
		    	...
		    	case 1:
		    		...
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo1';
		    	...
		    	case 2:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo2';
		    		
		    		//Set default demo pages
		    		$page_on_front = 4504; //Demo Homepage ID
		    	...
		    	case 3:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo3';
		    	...
		    	case 4:
		    		...
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo4';
		    	...
		    	case 5:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo5';
		    	...
		    	case 6:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo']."_woo.xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo6';
		    	...
		    	case 7:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo']."_woo.xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo7';
		    	...
		    	case 8:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo8';
		    	...
		    	case 9:
		    		...
		    		$import_filepath = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".xml" ;
		    		$styling_file = get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/".$_POST['demo'].".dat" ;
		    		$oldurl = 'https://themes.themegoods.com/grandportfolio/demo9';
	    ...
	    switch($_POST['demo'])
	    {
		    ...
		    case 2:
				...
				$locations['side-menu'] = $top_menu_id;
		    break;
		...
		if(class_exists('RevSlider'))
		{
			...
			switch($_POST['demo'])
	    	{
		    	case 99:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/home-slider.zip",
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/join.zip",
		    		);
		    	break;
		    	
		    	case 3:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/about-header.zip",
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/home-slider-3.zip",
		    		);
		    	break;
		    	
		    	case 4:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/home-slider-4.zip",
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/brand-ambrassador.zip",
		    		);
		    	break;
		    	...
		    	case 6:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/about-header-3.zip",
		    		);
		    	break;
		    	
		    	case 7:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/home-slider-6.zip",
		    		);
		    	break;
		    	
		    	case 9:
		    		$slider_array = array(
		    			get_template_directory() ."/cache/demos/importer/demo".$_POST['demo']."/home-slider-7.zip",
		    		);
		    	break;
	    	}
	...
	if (strlen($_POST['s'])>0) {
		...
		$querystr = "
	 	...
	 	if(!empty($pageposts))
	 	{
	 		...
	 		foreach($pageposts as $result_item) 
	 		{
				...
				switch($post_type)
				{
				    case 'galleries':
				    	...
				    	$post_type_title = esc_html__('Gallery', 'grandportfolio' );
				    break;
				    
				    case 'page':
				    default:
				    	$post_type_class = '<i class="fa fa-file-text-o"></i>';
				    	$post_type_title = esc_html__('Page', 'grandportfolio' );
				    break;
				    
				    case 'projects':
				    	$post_type_class = '<i class="fa fa-folder-open-o"></i>';
				    	$post_type_title = esc_html__('Projects', 'grandportfolio' );
				    break;
				    
				    case 'services':
				    	$post_type_class = '<i class="fa fa-star"></i>';
				    	$post_type_title = esc_html__('Service', 'grandportfolio' );
				    break;
				    
				    case 'clients':
				    	$post_type_class = '<i class="fa fa-user"></i>';
				    	$post_type_title = esc_html__('Client', 'grandportfolio' );
				    break;
				}
			...
			echo '<li class="view_all"><a href="javascript:jQuery(\'#searchform\').submit()">'.esc_html__('View all results', 'grandportfolio' ).'</a></li>';
...
function grandportfolio_contact_mailer() {
	...
	if (isset($_POST['your_name'])) {
		...
		$pp_contact_thankyou = esc_html__('Thank you! We will get back to you as soon as possible', 'grandportfolio' );
		...
		*/
		...
		{
			$contact_subject = esc_html__('[Email Contact]', 'grandportfolio' ).' '.get_bloginfo('name');
		}
		else
		{
	...
	{	
		for($i = $offset; $i <= $current_offset; $i++)
		{
			if(isset($images_arr[$i]))
			{
				...
				{
				    $image_caption.= '<a href="'.esc_url($grandportfolio_purchase_url).'" class="button ghost"><i class="fa fa-shopping-cart marginright"></i>'.esc_html__('Purchase', 'grandportfolio' ).'</a>';
				}
...
function grandportfolio_image_flow_xml() {
	...
	foreach($all_photo_arr as $photo_id)
	{
		...
		if(!empty($tg_full_image_caption))
		{
			...
			if(!empty($grandportfolio_purchase_url))
			{
				$image_caption.= '<br/><a href="'.esc_url($grandportfolio_purchase_url).'" class="button ghost"><i class="fa fa-shopping-cart marginright"></i>'.esc_html__('Purchase', 'grandportfolio' ).'</a>';
			}
...
function grandportfolio_get_captcha() {
	...
	{	
		...
		{
			echo esc_html__('Please enter correct captcha text', 'grandportfolio' );
		}
