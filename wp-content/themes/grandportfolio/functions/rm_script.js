...
jQuery(document).ready(function(){
	...
	jQuery('#pp_import_content_button').on( 'click', function(){
        ...
        jQuery.post(ajaxurl, data, function(response) {
            jQuery('.import_message').html('<div class="import_message_success">All done.</div>');
	...
	jQuery('#themegoods-envato-code-submit').on( 'click', function(){
		...
		if(envatoPurchaseCode.length != 36) {
			...
			if(jQuery('#pp_registration_section .tg_error').length == 0) {
				jQuery('<br style="clear:both;"/><div class="tg_error"><span class="dashicons dashicons-warning"></span>Purchase code is invalid</div>').insertAfter('#themegoods-envato-code-submit');
			}
