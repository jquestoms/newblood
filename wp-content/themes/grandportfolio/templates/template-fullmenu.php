...
<div class="mobile_menu_wrapper">
	<div class="fullmenu_wrapper">
			...
			<div class="logo_align">
	    	    <a id="custom_fullscreen_logo" class="logo_wrapper <?php if(!empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
	    	    	<?php
	    				...
	    				{
	    			?>
	    			<img src="<?php echo esc_url($tg_retina_fullscreen_menu_logo); ?>" alt="<?php echo esc_attr($image_alt); ?>" width="<?php echo esc_attr($image_width); ?>" height="<?php echo esc_attr($image_height); ?>"/>
	    			<?php
	    				}
	    				else
	    				{
		...
		<form method="get" name="searchform" id="searchform" action="<?php echo esc_url(home_url('/')); ?>/">
		    <div>
		    	<input type="text" value="<?php the_search_query(); ?>" name="s" id="s" autocomplete="off" placeholder="<?php esc_html_e('Search...', 'grandportfolio' ); ?>"/>
		    	<button>
		...
		<div class="fullscreen_portfolios">
			<h2 class="widgettitle"><?php esc_html_e('Recent Projects', 'grandportfolio' ); ?></h2>
