...
if(!empty($gallery_password) && (!isset($_SESSION['gallery_page_'.$current_page_id]) OR empty($_SESSION['gallery_page_'.$current_page_id])))
{
	...
	if(isset($_POST['gallery_password']))
	{
		...
		{
			$notice_text = esc_html__('Error Password is incorrect', 'grandportfolio' );
		}
		else
		{	
			    ...
			    <div class="overlay_gallery_wrapper">
				    <div class="overlay_gallery_border">
					    <div class="overlay_gallery_content">
					        ...
					        <p><strong><?php esc_html_e('To continue it please enter your password below', 'grandportfolio' ); ?></strong></p><br/>
					        ...
					        <form id="gallery_password_form" method="post" action="<?php echo get_permalink($current_page_id); ?>">
					            <input id="gallery_password" name="gallery_password" type="password" placeholder="<?php echo esc_attr(esc_html__('Password', 'grandportfolio' )); ?>"/><br/><br/>
					            <input type="submit" value="<?php echo esc_html__('Authenticate', 'grandportfolio' ); ?>" class="login_gallery"/>
					        </form>
					    </div>
				    </div>
			    </div>
