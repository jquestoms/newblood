...
elseif(is_archive())
{
    ...
    elseif ( !empty($term) ) : 
    	...
    	$page_title = esc_html__('Blog Archives', 'grandportfolio'); 
    endif;
...
{
	$page_show_title = 0;
	$page_title = esc_html__('Search', 'grandportfolio' );
	$term = 'search';
}
...
if(empty($page_show_title))
{
	...
	if(is_search())
	{
		$page_tagline = esc_html__('Search Results for ', 'grandportfolio' ).get_search_query();
	}
