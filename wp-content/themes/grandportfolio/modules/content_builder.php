...
?>
	...
	<strong><?php esc_html_e('Enable Content Builder', 'grandportfolio' ); ?></strong>
	<hr class="pp_widget_hr">
	<div class="pp_widget_description"><?php esc_html_e('To build this page using content builder, please enable this option.', 'grandportfolio' ); ?></div><br/>
	...
	<?php if((empty($ppb_enable) OR isset($_GET['gutenberg-editor'])) && !isset($_GET['classic-editor'])) { ?>
	<script>
		jQuery(document).ready(function(){
			jQuery('#content_metabox').hide();
		});
	</script>
	<?php } ?>
	...
	<div id="ppb_page_content" class="hidden">
		...
		<div class="ppb_page_action">
			<a id="ppb_preview_page" class="tooltipster" title="<?php esc_html_e('Preview Page', 'grandportfolio' ); ?>" data-action="<?php echo esc_url(admin_url('admin-ajax.php?action=grandportfolio_ppb_preview_page_set_data')); ?>" data-preview="<?php echo esc_url(admin_url('admin-ajax.php?action=grandportfolio_ppb_preview_page&ppb_post_type=page&page_id='.$post->ID)); ?>" data-page="<?php echo esc_attr($post->ID); ?>"><i class="fa fa-search"></i></a>
			<!-- a id="ppb_save_template" class="tooltipster" title="<?php esc_html_e('Save as template', 'grandportfolio' ); ?>"><i class="fa fa-copy"></i></a -->
			<a id="ppb_save" class="tooltipster" title="<?php esc_html_e('Save Changes', 'grandportfolio' ); ?>"><i class="fa fa-floppy-o"></i></a>
		</div>
		<br class="clear"/>
		<?php
			}
	...
	<a id="ppb_sortable_preview_button" class="button" style="float:left;" href="<?php echo esc_url(admin_url('admin-ajax.php?action=grandportfolio_ppb_demo_preview')); ?>"><?php esc_html_e('Preview', 'grandportfolio' ); ?></a>
	<a id="ppb_sortable_add_button" class="button button-primary" style="float:left;"><?php esc_html_e('Add', 'grandportfolio' ); ?></a>
	...
	?>
			<li id="<?php echo esc_attr($ppb_form_item); ?>" class="ui-state-default <?php echo esc_attr($ppb_form_item_size[0]); ?> <?php echo esc_attr($ppb_form_item_data_obj->shortcode); ?>" data-current-size="<?php echo esc_attr($ppb_form_item_size[0]); ?>">
				<div class="size">
					<a href="javascript:;" title="<?php esc_html_e('Increase Size', 'grandportfolio' ); ?>" class="ppb_plus button">+</a>
					<a href="javascript:;" title="<?php esc_html_e('Decrease Size', 'grandportfolio' ); ?>" class="ppb_minus button">-</a>
				</div>
				...
				<div class="item_action">
					<a href="javascript:;" class="ppb_remove tooltipster" title="<?php esc_html_e('Remove', 'grandportfolio' ); ?>"><i class="fa fa-trash"></i></a>
					<a data-rel="<?php echo esc_attr($ppb_form_item); ?>" href="<?php echo esc_url(admin_url('admin-ajax.php?action=grandportfolio_ppb_preview&ppb_post_type=page&title='.urlencode($ppb_shortocde_title).'&page_id='.$post->ID.'&shortcode='.$ppb_form_item_data_obj->shortcode.'&rel='.$ppb_form_item)); ?>" class="ppb_preview tooltipster" title="<?php esc_html_e('Preview', 'grandportfolio' ); ?>"><i class="fa fa-search"></i></a>
					<a href="javascript:;" class="ppb_duplicate tooltipster" title="<?php esc_html_e('Duplicate', 'grandportfolio' ); ?>"><i class="fa fa-copy"></i></a>
					<a data-rel="<?php echo esc_attr($ppb_form_item); ?>" href="<?php echo esc_url(admin_url('admin-ajax.php?action=grandportfolio_ppb&ppb_post_type=page&shortcode='.$ppb_form_item_data_obj->shortcode.'&rel='.$ppb_form_item.'&width=800&height=900')); ?>" class="ppb_edit tooltipster" title="<?php esc_html_e('Edit', 'grandportfolio' ); ?>"><i class="fa fa-edit"></i></a>
				</div>
	...
	<div id="ppb_import_tab">
		<ul>
			<li><a href="#tabs-import"><?php esc_html_e('Import', 'grandportfolio' ); ?></a></li>
			<li><a href="#tabs-export"><?php esc_html_e('Export', 'grandportfolio' ); ?></a></li>
		</ul>
		
		<div id="tabs-import">
			<strong><?php esc_html_e('Import Page Content Builder', 'grandportfolio' ); ?></strong>
			<div class="pp_widget_description"><?php esc_html_e('Choose the import file. *Note: Your current content builder content will be overwritten by imported data', 'grandportfolio' ); ?></div><br/>
		...
		<div id="tabs-export">
			<strong><?php esc_html_e('Export Current Page Content Builder', 'grandportfolio' ); ?></strong>
			<div class="pp_widget_description"><?php esc_html_e('Click to export current content builder data. *Note: Please make sure you save all changes and no "unsaved" module', 'grandportfolio' ); ?></div><br/>
	...
	?>
			jQuery(window).bind('beforeunload', function(){
				...
				{
			    	return '<?php esc_html_e('There are unsaved content builder settings', 'grandportfolio' ); ?>';
			    }
			});
	});
	</script>
